...
public class AStar<S extends HashableState, A> {
    public static <S extends HashableState, A> Solution<S, A> search(HeuristicProblem<S, A> prob) {
        ...
        gScore.put(start.computeZobristHash(), 0.0);
...
//                System.out.println("ACtion : " + action);
                ...
                long hash = nextState.computeZobristHash();
                double newCost = current.g + prob.cost(current.state, action);

//                 If we already found this state with a lower cost, skip it
                if (gScore.containsKey(hash) && newCost >= gScore.get(hash)) {
                    continue;
                }

                gScore.put(hash, newCost);
